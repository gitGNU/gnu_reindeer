...
ren_color_format_sizeof (RenColorFormat format, RenType type)
{
	switch (format)
	{
		case REN_COLOR_FORMAT_RGB:
			return 3 * ren_type_sizeof (type);
		case REN_COLOR_FORMAT_RGBA:
			return 4 * ren_type_sizeof (type);
		case REN_COLOR_FORMAT_DEPTH:
			return 1 * ren_type_sizeof (type);
		default:
			return 0;
	}
}
